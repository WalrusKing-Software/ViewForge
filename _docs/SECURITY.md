...
| PF-9 | Re-opening ViewForge-owned output (ADR-032) reads a chosen `.kt` **only to recognise it**, never to derive IR: the `// Generated by ViewForge` header is a weak hint, ownership is the nearest ancestor `.viewforge/manifest.json` (loaded totally per ADR-029 — corrupt ⇒ nothing owned ⇒ refuse), and IR comes solely from the `.viewforge/project.vforge` sidecar loaded through this section's hardened `.vforge` path (PF-1…PF-8 apply unchanged). A forged header at most points the importer at a sidecar that must still survive strict parse; header presence is never licence to parse arbitrary Kotlin. |
| PF-10 | The chosen `.kt` and the walk to its ancestor `.viewforge/` are resolved and canonicalised before use; reject traversal, absolute/UNC/drive-relative forms, and symlinks that escape the located `.viewforge/` root (same discipline as PF-5 and FW-2/FW-4). Fail loud on anything outside the recognised owned tree. |
| PF-11 | Read-only data binding (ADR-034) introduces **no evaluation path**. A `PropValue.StateBinding` `path` is a **dotted identifier** navigated by structural lookup over declared state — a `Screen.state` or, since schema 5, a `ComponentDef.state` (Amendment #266) — never parsed or run as Kotlin; an unresolved path renders a placeholder and marks the node unverified (PF-6 discipline), never dynamic dispatch. Sample data is typed literals only (the `PropValue.Literal` trust boundary), bounded by the same file/size limits as any document (PF-2 — including the renderer's per-repeat row cap). State field names and binding identifiers are validated as legal Kotlin identifiers (GC-3). |
...
sidecar `.vforge` is the source of truth and rides the exact hardened loader above. The threat surface it
adds is ordinary file *reading* (path handling, PF-10) plus one more `.vforge` parse (already covered),
