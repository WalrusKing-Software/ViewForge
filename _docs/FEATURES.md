...
| C11 | Alignment guides / snapping | P2 | Guides appear when edges align during drag. (#118: **shipped as static guides**, reinterpreted for the container-layout canvas — a View → "Alignment guides" toggle draws a line wherever the selected node's edges/centre line up with a sibling or the parent (pure `alignmentGuides` in `Selection.kt`, reusing the #116 content-space transform). Free-move **snapping during a drag** presumes absolute positioning the canvas doesn't have (drag is reparent-only, no node x/y, no offset modifier), so it's split to **#129** (blocked, needs a positioning model + ADR).) |
| C12 | Measure/spacing overlay | P2 | Hold a modifier key to show spacing between selection and neighbors. (#119: **shipped** — hold **M** over the canvas to overlay the gaps from the primary selection to its parent container's four inner edges, labelled in dp. Editor-only, reuses the #116 content-space transform; pure `measureGaps` in `Selection.kt`, held-key tracked like space-pan via `EditorState.isMeasuring`. v1 measures to the container edges; sibling-aware measuring is #127.) |
| C13 | Interactive preview mode | P2 | Toggle to interact with the real UI (buttons clickable) rather than edit it. (#120: **shipped** — a View → Interactive preview toggle (+ toolbar ▶ Preview button, command palette entry) flips the canvas into run mode. The editor removes the selection overlay so pointer events reach the live tree (buttons ripple, scrollables scroll), and the renderer's stateful inputs (TextField/OutlinedTextField/Slider/Checkbox/Switch) get real local state + live callbacks so they are editable/toggleable. Threaded via `CanvasRenderer.Render(interactive)` → `RenderContext.interactive` (defaults false, so codegen and the fidelity tests are untouched). Transient `EditorState.interactivePreview`, never serialized, reset on document swap. Selection is preserved, so leaving preview restores the outlines. Space-drag pan is inactive in preview (overlay off); keyboard/menu zoom still works.) |
...
> lockstep. `Image` sources are `ResourceRef`s picked from the project's assets; the Gradle export
...
| S4 | Command palette | P2 | (#123: **shipped** — Ctrl+Shift+P (Cmd on macOS) opens a focusable-popup fuzzy launcher over the editor's actions. The catalog is assembled in the shell mirroring `AppMenuBar` (File/Edit/View, reusing the menu models for enablement) plus dynamic "Go to screen" and cycle-gated "Add \<component\>" entries from the live palette; a disabled command shows greyed and can't be invoked. Matching/ranking is a pure Compose-free `rankCommands` (exact > prefix > substring > subsequence), unit-tested like the layers-panel tree search (#122). ↑/↓ navigate skipping disabled rows, Enter invokes, Esc/outside-click dismiss.) |
